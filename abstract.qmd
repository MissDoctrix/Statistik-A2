...
**Für die Abgabe aktueller GitHub Hash:** ead2de2c7085b955cca3ce6db265134d9240202b
...
Forschungsfrage: Wie gehen Personen in diesem Kontext mit unterschiedlichen Feedback-Arten um?

Daten wurden bereits gefiltert von Lehrveranstaltungsteam, keine weitere Filterung vorgenommen.
...
# Gruppierungsvariable erstellen: Evaluativ vs. Empfehlend
data$group <- NA
...
long_data <- data %>%
  select(participantId, group, workload_mean_E, workload_mean_R) %>%
  pivot_longer(cols = c(workload_mean_E, workload_mean_R),
               names_to = "variable",
               values_to = "workload") %>%
  filter(!is.na(workload)) %>%
  mutate(group = ifelse(variable == "workload_mean_E", "Evaluativ", "Empfehlend"))
...
desc_stats <- long_data %>%
  ...
  summarise(
    ...
    min = min(workload, na.rm = TRUE),
    max = max(workload, na.rm = TRUE)
  )

# Tabelle ausgeben
kable(desc_stats,
      caption = "Deskriptive Statistik der subjektiven Belastung nach KI-Typ",
      booktabs = TRUE) %>%
  kable_styling()
...
plot_data <- long_data %>%
  ...
  summarise(
    ...
    sd_workload = sd(workload, na.rm = TRUE),
    n = n(),
    se = sd_workload / sqrt(n),
    ci_lower = mean_workload - qt(0.975, df = n-1) * se,
    ci_upper = mean_workload + qt(0.975, df = n-1) * se
  )

# Dot-and-Whisker-Plot erstellen
ggplot(plot_data, aes(x = group, y = mean_workload)) +
  geom_point(size = 4) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.1) +
  labs(
    title = "Subjektive Belastung nach KI-Typ (evaluativ vs. empfehlend)",
    x = "KI-Typ",
    y = "Subjektive Belastung (0–200)"
  ) +
...
ggplot(data, aes(x = DLR_KI, y = Trust)) +
  ...
  labs(
    title = "Zusammenhang zwischen mentaler Belastung (DLR) und Vertrauen",
    x = "Mentale Belastung (DLR-Skala, mit KI)",
    y = "Vertrauen in das KI-System"
  ) +
  theme_minimal(base_size = 14)

# Beispiel Korrelations-Test
